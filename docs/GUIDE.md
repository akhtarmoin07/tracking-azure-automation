...
`enable_audit_schedules=true` requests automatic activation after checks. `enable_user_access=false` keeps publication off until tenant prerequisites and pilot decisions are complete; setting it true requests publication in the same flow, with an SSO configuration check. Only expected access/schedule/alert changes are permitted in the activation plan.
